...
from groq import AsyncGroq, APIConnectionError, APITimeoutError, AuthenticationError, RateLimitError, APIStatusError
...
# Configure logging
logger = logging.getLogger("aithera.groq_client")
...
class GroqConfigError(GroqProviderError):
    """Exception raised when Groq configuration is invalid or missing."""
    pass


class GroqAuthError(GroqProviderError):
    """Exception raised when Groq authentication fails."""
    pass


class GroqTransientError(GroqProviderError):
    """Exception raised when Groq transient error retries are exhausted."""
    pass


class GroqNonTransientError(GroqProviderError):
    """Exception raised for non-transient Groq errors."""
    pass
...
class GroqClient(BaseProvider):
    """Council provider interface client for Groq models (e.g. LLaMA)."""
    ...
    def __init__(self, config: Optional[ProviderConfig] = None) -> None:
        """Initialize the Groq client with configuration, falling back to defaults if not provided."""
        # Load model name from environment or config
        env_model = os.getenv("GROQ_MODEL", self.DEFAULT_MODEL)
        if config is None:
    ...
    def validate_config(self) -> None:
        """Validate that Groq client has required configuration."""
        if not self.api_key or not self.api_key.strip():
            raise GroqConfigError("Groq API key is missing or empty.")
    ...
    def _build_prompt_from_template(
        ...
        try:
            ...
            raise GroqProviderError(f"Failed to build prompt from template: {str(e)}") from e
    ...
    async def generate_response(
        ...
        **kwargs: Any,
        ...
        """Generate a prompt response using Groq.
        ...
        Returns:
            Dict[str, Any]: The raw Groq API response dictionary.
        """
        ...
        try:
            ...
            async for attempt in AsyncRetrying(
                ...
                with attempt:
                    ...
                    # Add response_time to raw_response so ResponseNormalizer can extract it
                    raw_response["response_time"] = duration
                    return raw_response

        except AuthenticationError as e:
            logger.error("Authentication failed: %s", str(e))
            raise GroqAuthError(f"Groq Authentication failed: {str(e)}") from e
        except (APIConnectionError, APITimeoutError, RateLimitError) as e:
            logger.error("Transient error encountered: %s", str(e))
            raise GroqTransientError(f"Groq transient error: {str(e)}") from e
        except APIStatusError as e:
            logger.error("API error status code %d: %s", e.status_code, str(e))
            if e.status_code >= 500:
                raise GroqTransientError(f"Groq server error: {str(e)}") from e
            else:
                raise GroqNonTransientError(f"Groq non-transient API error: {str(e)}") from e
        except Exception as e:
            logger.error("Unexpected error occurred: %s", str(e))
            raise GroqProviderError(f"Unexpected Groq provider error: {str(e)}") from e
