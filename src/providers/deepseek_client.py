...
# Configure logging
logger = logging.getLogger("aithera.deepseek_client")
...
class DeepSeekProviderError(Exception):
    """Base exception for DeepSeek provider errors."""
    pass


class DeepSeekConfigError(DeepSeekProviderError):
    """Exception raised when DeepSeek configuration is invalid or missing."""
    pass


class DeepSeekAuthError(DeepSeekProviderError):
    """Exception raised when DeepSeek authentication fails."""
    pass


class DeepSeekTransientError(DeepSeekProviderError):
    """Exception raised when DeepSeek transient error retries are exhausted."""
    pass


class DeepSeekNonTransientError(DeepSeekProviderError):
    """Exception raised for non-transient DeepSeek errors."""
    pass
...
class DeepSeekClient(BaseProvider):
    """Council provider interface client for DeepSeek models."""
    ...
    def __init__(self, config: Optional[ProviderConfig] = None) -> None:
        """Initialize the DeepSeek client with configuration, falling back to defaults if not provided."""
        env_model = os.getenv("DEEPSEEK_MODEL", self.DEFAULT_MODEL)
        if config is None:
        ...
        self.api_key = self.validate_api_key("DEEPSEEK_API_KEY")
        try:
    ...
    def validate_config(self) -> None:
        """Validate that DeepSeek client has required configuration."""
        if not self.api_key or not self.api_key.strip():
            raise DeepSeekConfigError("DeepSeek API key is missing or empty.")
    ...
    def _build_prompt_from_template(
        ...
        template_path = root_dir / "prompts" / "deepseek_logic.txt"
        ...
        try:
            ...
            raise DeepSeekProviderError(f"Failed to build prompt from template: {str(e)}") from e
    ...
    async def generate_response(
        ...
        **kwargs: Any,
        ...
        """Generate a prompt response using DeepSeek.
        ...
        logger.info("Request start - Provider: DeepSeek, Model: %s", model)
        ...
        try:
            ...
            async for attempt in AsyncRetrying(
                ...
                with attempt:
                    ...
                    logger.info("Sending DeepSeek API request (attempt %d/3)...", attempt_num)
            ...
            raise DeepSeekAuthError(f"DeepSeek Authentication failed: {str(e)}") from e
        except (APIConnectionError, APITimeoutError, RateLimitError) as e:
            logger.error("Transient error encountered: %s", str(e))
            raise DeepSeekTransientError(f"DeepSeek transient error: {str(e)}") from e
        except APIStatusError as e:
            logger.error("API error status code %d: %s", e.status_code, str(e))
            if e.status_code >= 500:
                raise DeepSeekTransientError(f"DeepSeek server error: {str(e)}") from e
            else:
                raise DeepSeekNonTransientError(f"DeepSeek non-transient API error: {str(e)}") from e
        except Exception as e:
            logger.error("Unexpected error occurred: %s", str(e))
            raise DeepSeekProviderError(f"Unexpected DeepSeek provider error: {str(e)}") from e
