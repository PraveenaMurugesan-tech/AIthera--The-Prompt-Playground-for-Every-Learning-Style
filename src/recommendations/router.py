...
@router.get("/practice-questions/generate", response_model=schemas.PracticeQuestionsResponse)
async def generate_more_practice_questions(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> schemas.PracticeQuestionsResponse:
    """
    Generate and return a new list of practice questions based on the user's prompt history.
    """
    try:
        # Fetch user's past prompts
        prompt_history = get_user_prompt_requests(db=db, user_id=current_user.id)
        ...
        # Call the generator service
        questions = await recommendation_generator.generate_more_practice_questions(prompt_history)
        
        return schemas.PracticeQuestionsResponse(practiceQuestions=questions)
    except Exception as e:
        error_msg = str(e)
        if "429" in error_msg or "RESOURCE_EXHAUSTED" in error_msg:
            raise HTTPException(
