...
def mock_env(monkeypatch):
    ...
    monkeypatch.setenv("DEEPSEEK_TIMEOUT", "10")
...
async def test_successful_response(client):
    ...
    with patch("src.providers.deepseek_client.AsyncOpenAI") as mock_openai:
...
async def test_authentication_failure(client):
    with patch("src.providers.deepseek_client.AsyncOpenAI") as mock_openai:
        ...
        mock_request = httpx.Request("POST", "https://api.deepseek.com")
...
async def test_rate_limit_handling(client):
    with patch("src.providers.deepseek_client.AsyncOpenAI") as mock_openai:
        ...
        with pytest.raises(DeepSeekTransientError):
            await client.generate_response(prompt="Hello")
...
async def test_client_error_non_transient(client):
    with patch("src.providers.deepseek_client.AsyncOpenAI") as mock_openai:
        ...
        with pytest.raises(DeepSeekNonTransientError):
            await client.generate_response(prompt="Hello")
